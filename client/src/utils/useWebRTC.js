...
export function useWebRTC(username, setIncomingCall, setInCall, setIsRinging, startTimer, stopTimer) {
  ...
  const cleanup = () => {
    ...
    if (pcRef.current) {
      pcRef.current.getSenders().forEach((sender) => sender.track?.stop()); // ✅ stop mic
      pcRef.current.close();
      pcRef.current = null;
    }
  };
